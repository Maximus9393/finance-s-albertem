      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-10 md:mb-16">
          ...
          animationDelay: `${index * 0.1}s`
              ...
              <div className={`inline-flex items-center justify-center w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br ${service.gradient} rounded-lg md:rounded-xl mb-3 md:mb-4 group-hover:scale-110 transition-transform duration-300 flex-shrink-0`}>
              ...
              <p className="text-white/90 mb-3 md:mb-4 leading-relaxed text-sm flex-grow">{service.description}</p>
              ...
              <ul className="space-y-1.5 md:space-y-2 mb-4 md:mb-6 flex-grow">
                {service.features.map((feature, featureIndex) => <li key={featureIndex} className="flex items-start space-x-2">
                    <div className="w-1.5 h-1.5 bg-white rounded-full mt-1.5 md:mt-2 flex-shrink-0"></div>
                    <span className="text-white text-xs md:text-sm leading-relaxed">{feature}</span>
                  </li>)}
              </ul>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
          <div className="bg-card/50 rounded-xl p-5 md:p-6 border border-border/30 hover:border-primary/30 transition-all duration-300">
            <Calculator className="w-7 h-7 md:w-8 md:h-8 text-white mb-3 md:mb-4" />
            <h4 className="text-base md:text-lg font-semibold text-white mb-2">Finanční plánování</h4>
            <p className="text-white/90 text-xs md:text-sm">Strategické plánování vašich financí pro dosažení životních cílů.</p>
          </div>
          
          <div className="bg-card/50 rounded-xl p-5 md:p-6 border border-border/30 hover:border-primary/30 active:scale-[0.98] md:hover:scale-105 hover:shadow-lg hover:bg-card/60 transition-all duration-300 cursor-pointer" onClick={navigateToReality}>
            <Home className="w-7 h-7 md:w-8 md:h-8 text-white mb-3 md:mb-4" />
            <h4 className="text-base md:text-lg font-semibold text-white mb-2">Reality</h4>
            <p className="text-white/90 text-xs md:text-sm">Ve spolupráci s renomovanými realitními makléři jsme Vám schopni zajistit profesionální servis při prodeji nebo pronájmu Vaší nemovitosti.</p>
          </div>
          
          <div className="bg-card/50 rounded-xl p-5 md:p-6 border border-border/30 hover:border-primary/30 transition-all duration-300">
            <FileText className="w-7 h-7 md:w-8 md:h-8 text-white mb-3 md:mb-4" />
            <h4 className="text-base md:text-lg font-semibold text-white mb-2">Daňové poradenství</h4>
            <p className="text-white/90 text-xs md:text-sm">Optimalizace daňových povinností a využití úlev.</p>
          </div>
        </div>
      </div>
    </section>;
